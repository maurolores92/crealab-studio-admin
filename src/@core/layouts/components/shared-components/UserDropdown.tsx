...
const UserDropdown = (props: Props) => {
  ...
  return (
    <Fragment>
      <Box sx={{ display: 'flex', ml: 2.5, mr: 2.5, alignItems: 'flex-end', flexDirection: 'column' }}>
        ...
        <Box sx={{ py: 1.75, px: 6 }}>
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            ...
            <Box sx={{ display: 'flex', ml: 2.5, alignItems: 'flex-start', flexDirection: 'column' }}>
              <Typography sx={{ fontWeight: 500 }}>{`${user?.name} ${user?.lastName}`}</Typography>
              <Typography variant='body2'>{user?.role.name}</Typography>
            </Box>
          </Box>
        </Box>
        <Divider sx={{ my: theme => `${theme.spacing(2)} !important` }} />
         <MenuItemStyled sx={{ p: 0 }} onClick={() => handleDropdownClose('/perfil')}>
          <Box sx={styles}>
