...
const CurrencyList = () => {
  ...
  const [currencies, setCurrencies] = useState<IDataList>({...defaultDataList});


  const get = useCallback(async() => {
    const result: any = await apiConnector.get('/currency', paginationModel);
    setCurrencies(result);
  }, [setCurrencies, paginationModel]);
  ...
  const onDelete = async() => {
    try {
      await apiConnector.remove(`/currency/${selected.id}`);
      setOpenDelete(false);
  ...
  return <>
    ...
    <Grid>
      <Grid item lg={12} xs={12}>
        <Card>
          <CardHeader title='Monedas' action={<>
            <Button
              variant="contained"
    ...
    }
    {
      ...
      action={onDelete}
      >
        <Typography fontSize={14}>¿Está seguro que desea eliminar la moneda <strong>{selected.name}</strong>?</Typography>
      </ConfirmDeleteDialog>
    }
  </>
}
