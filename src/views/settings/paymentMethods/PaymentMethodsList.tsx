...
const PaymentMethodsList = () => {
  ...
  const [paymentMethods, setPaymentMethods] = useState<IDataList>({...defaultDataList});



  const get = useCallback(async() => {
    const result: any = await apiConnector.get('/paymentMethod', paginationModel);
    setPaymentMethods(result);
  }, [setPaymentMethods, paginationModel]);
  ...
  const onDelete = async() => {
    try {
      await apiConnector.remove(`/paymentMethod/${selected.id}`);
      setOpenDelete(false);
  ...
  return <>
    ...
    <Grid>
      <Grid item lg={12} xs={12}>
        <Card>
          <CardHeader title='Metodos de Pago' action={<>
            <Button
              variant="contained"
